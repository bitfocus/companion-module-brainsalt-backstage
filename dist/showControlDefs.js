...
export function scVarId(domain, name, field) {
...
function nodeEndpoints(sc, filter) {
    const out = [];
...
function nodeTargetOption(sc, filter) {
    const choices = nodeEndpoints(sc, filter).map((ep) => ({
        id: ep.name,
...
function targetChoices(sc, domain, filter) {
    const out = [];
    for (const name of sc.entities.get(domain)?.keys() ?? []) {
        if (filter && !filter(sc.entities.get(domain).get(name)))
            continue;
        out.push({ id: name, label: name });
    }
    return out;
}
function targetOption(sc, domain, label, filter) {
...
// ─── Variable values ───
export function scVariableValuesFor(sc, domain, name) {
    const vals = {};
    const v = (field, value) => {
    ...
    switch (domain) {
        case 'step': {
            const s = sc.getStatus('step', name);
            if (!s)
                break;
            v('state', s.state);
            v('step', s.step);
        ...
        case 'bcast': {
            const s = sc.getStatus('bcast', name);
            if (!s)
                break;
        ...
        case 'output':
        case 'mixer': {
            const s = sc.getStatus(domain, name);
            if (!s)
                break;
        ...
        case 'input': {
            const s = sc.getStatus('input', name);
            if (!s)
                break;
        ...
        case 'node': {
            const s = sc.getStatus('node', name);
            if (!s || s.dir === 'in')
                break;
...
export function scAllVariableValues(sc) {
    const vals = {};
...
// ─── Definitions ───
export function buildShowControlDefs(sc, label) {
    if (!sc)
...
function buildActions(sc) {
    ...
    actions['sc_seq_tempo_adjust'] = {
        ...
        callback: async (evt) => {
            const s = sc.getStatus('seq', String(evt.options.target));
    ...
    actions['sc_bcast_enable'] = {
        ...
        callback: async (evt) => {
            ...
            if (evt.options.mode === 'toggle') {
                const s = sc.getStatus('bcast', target);
    ...
    for (const domain of ['output', 'mixer']) {
        ...
        actions[`sc_${domain}_volume`] = {
            ...
            callback: async (evt) => {
                ...
                const args = { volume: Number(evt.options.volume) };
                if (channel >= 0)
    ...
    const sendNode = (target, value) => sc.sendAction('node', 'send', target, { value });
    const nodeEp = (target) => sc.getStatus('node', target);
...
function buildFeedbacks(sc) {
    ...
    feedbacks['sc_step_state'] = {
        ...
        callback: (evt) => {
            const s = sc.getStatus('step', String(evt.options.target));
    ...
    feedbacks['sc_bcast_enabled'] = {
        ...
        options: [targetOption(sc, 'bcast', 'Broadcast Scheduler')],
        callback: (evt) => {
            const s = sc.getStatus('bcast', String(evt.options.target));
    ...
    feedbacks['sc_cal_state'] = {
        ...
        callback: (evt) => {
            const s = sc.getStatus('cal', String(evt.options.target));
            return s?.state === evt.options.state;
        },
    };
    for (const domain of ['output', 'mixer']) {
        ...
        feedbacks[`sc_${domain}_muted`] = {
            ...
            callback: (evt) => {
                const s = sc.getStatus(domain, String(evt.options.target));
                return s?.muted === true;
            },
        };
    ...
    feedbacks['sc_input_source'] = {
        ...
        callback: (evt) => {
            const s = sc.getStatus('input', String(evt.options.target));
    ...
    feedbacks['sc_node_equals'] = {
        ...
        callback: (evt) => {
            const s = sc.getStatus('node', String(evt.options.target));
    ...
    feedbacks['sc_node_bool_on'] = {
        ...
        callback: (evt) => {
            return nodeBool(sc.getStatus('node', String(evt.options.target)));
        },
    };
    feedbacks['sc_node_enum_matches'] = {
        ...
        callback: (evt) => {
            const ep = sc.getStatus('node', String(evt.options.target));
...
function buildVariables(sc) {
    const defs = [];
    const add = (domain, name, field, label) => {
...
function buildPresets(sc, LBL) {
    const presets = {};
    const white = combineRgb(255, 255, 255);
    const pid = (domain, name, suffix) => `sc_${domain}_${san(name)}_${suffix}`;
    const varRef = (domain, name, field, label) => `$(${label}:${scVarId(domain, name, field)})`;
    for (const name of sc.entities.get('step')?.keys() ?? []) {
        const transport = (verb, text, bg, feedbackState) => {
...
//# sourceMappingURL=showControlDefs.js.map